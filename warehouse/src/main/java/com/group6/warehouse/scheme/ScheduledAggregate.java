...
//    @Scheduled(cron = "0 35 0 * * 2,4,6")
    ...
    public void executeAggregateJar() {
        try {
            ...
            if (isFailLogDetected()) {
                System.out.println("Fail log detected for Transform Aggregate. Retrying...");
                retryTask(new IOException("Fail log detected for Transform Aggregate"));
            } else {
    ...
    private boolean isFailLogDetected() {
        ...
        String sql = "SELECT status FROM logs WHERE task_name = 'Transform Aggregate' ORDER BY id DESC LIMIT 1";
    ...
    public void retryTask(Exception initialException) {
        while (retryCount < MAX_RETRIES) {
            try {
                ...
                if (isFailLogDetected()) {
                    System.out.println("Fail log detected for Transform Aggregate again. Retrying...");
