...
public class LoadToStaging {
    public static void main(String[] args) {
        ...
        final String DB_CONTROL_PASSWORD = ConfigReader.get("db.control.password");
        ...
        try {
            ...
            String filename = details[1].replace("Filename: ", ""); // Phần tử thứ 2 của chuỗi
            ...
            String filePath = String.format("%s\\%s%s.csv", directoryFile, filename, formattedDate);
    ...
    private static String getCrawlFileDetails(Connection dbControlConnection, LocalDate dateToCheck) {
        ...
        String logQuery = "SELECT df.directory_file, df.filename "
                + "FROM logs l "
        ...
        try (PreparedStatement statement = dbControlConnection.prepareStatement(logQuery)) {
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                if (resultSet.next()) {
                    ...
                    return "Directory: " + directoryFile + ", Filename: " + filename;
                }
            }
        } catch (SQLException e) {
